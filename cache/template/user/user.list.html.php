...
<div class="mini-fit" style="padding:1px 3px 3px;">
    <div id="gridCell" class="mini-datagrid" style="width:100%;height:100%;"  sizeList="[10,20,50,100]" pageSize="20"
        url="/user/user/init?action=grid&do=<?php echo $this->_var['doact']; ?><?php if ($this->_var['isSecurity'] == '1'): ?>&isSecurity=<?php echo $this->_var['isSecurity']; ?><?php endif; ?>" idField="user_id"  onrowdblclick="onRowDblClick"
        multiSelect="true" <?php if ($this->_var['doact'] != 'search'): ?>allowCellEdit="true" allowCellSelect="true"<?php endif; ?> contextMenu="#gridMenu"  headerContextMenu="#headerMenu"
        >
    ...
    <ul id="gridMenu" class="mini-contextmenu" onbeforeopen="onBeforeOpen">              
	    <li name="edit" iconCls="icon-edit" onclick="onEdit">修改用户</li>
        <li name="modifyPasswd" iconCls="icon-edit" onclick="modifyPasswd">重置密码</li>
        <li name="modifyPayPasswd" iconCls="icon-edit" onclick="modifyPayPasswd">重置交易密码</li>
        <li name="applyUser" iconCls="icon-edit" onclick="applyUser">修改用户状态</li>
        <li name="addWhiteList" iconCls="icon-add" onclick="addWhiteList();">添加到提现白名单</li>
        <li name="addRfWhiteList" iconCls="icon-add" onclick="addRfWhiteList();">添加到推荐白名单</li>
        <li name="setCustomerManager" iconCls="icon-add" onclick="setCustomerManager">设置客户经理</li> 
        <li name="editRf" iconCls="icon-edit" onclick="editRfUser">修改用户推荐人</li> 
    </ul>
</div>
<?php if ($this->_var['doact'] == 'search'): ?>
...
function onEdit(e) {
	...
	if (row) {
		...
		urlStr="/user/user/edit?id="+row.user_id;
		mini.open({
			url: urlStr,
			title: title, width: width, height:height,
			ondestroy: function (action) {
...
function modifyPayPasswd(e) {
	var row = grid.getSelected();
	if (row) {
		var width=300,height=200,title='重置交易密码';
		urlStr="/user/user/modifyPayPasswd?id="+row.user_id;
		mini.open({
			url: urlStr,
			title: title, width: width, height:height,
			ondestroy: function (action) {
...
function applyUser(e) {
	var row = grid.getSelected();
	if (row) {
		var width=300,height=200,title='修改用户状态';
		urlStr="/user/user/applyUser?id="+row.user_id;
		mini.open({
			url: urlStr,
			title: title, width: width, height:height,
			ondestroy: function (action) {
...
function editRfUser(e) {
	var row = grid.getSelected();
	if (row) {
		var width=300,height=150,title='修改用户推荐人';
		urlStr="/user/user/editRfUser?id="+row.user_id;
		mini.open({
			url: urlStr,
			title: title, width: width, height:height,
			ondestroy: function (action) {
...
function setCustomerManager(e) {

	var rows = grid.getSelecteds(),_ids=new Array();
	
	if(rows.length<1) return;
	for(var i=0;i<rows.length;i++){
		_ids[i]=rows[i].user_id;
	}
	var ids=_ids.join(',');
	if(ids=='') return;
	
	var width=300,height=150,title='设置客户经理';
	urlStr="/user/user/setUser?ids="+ids;
	mini.open({
		url: urlStr,
		title: title, width: width, height:height,
		ondestroy: function (action) {
...
function addWhiteList() {
	...
	if (row) {
		var ids=row.user_id;
		var urlStr="/user/whitelist/add";
		var msg = "确定把此用户添加到提现白名单？";
		...
		mini.confirm(msg, "提示",	function(action){
			...
			utils.ajax(urlStr,{ids:ids},callback,"POST","json");
		});
	}             
}

function addRfWhiteList(){
	var row = grid.getSelected();
	if (row) {
		mini.showMessageBox({
			title: "设置推荐白名单",
			...
			html: '<div id="form-rfwhitelist" style="padding:10px 30px">\
					...
					<p><label> <input name="type[]" value="invest" type="checkbox" /><strong>投资奖励</strong></label></p>\
					<p><label> <input name="type[]" value="stock" type="checkbox" /><strong>配资奖励</strong></label></p>\
				  </div>'
				 ,
			callback: function (action) {
				if(action=='ok'){
					$.post('/user/rfwhitelist/add',$("#form-rfwhitelist input").serialize(),function(ret){
						alert(ret.msg);
					},'json');
				}
			}
