...
class LtAutoloader
{
	...
	protected function addClass($className, $file)
	{
		...
		{
			$this->storeHandle->add($key, $file);//ps  $key  ltconfig   $file D:\xamp\htdocs\janfly1\branches\common\extend\alipay-sdk-PHP\lotusphp_runtime\Config.php
	...
	protected function addFileMap($file)
	{
		...
		$libNames = array();//p($this->fileStore->storeDir);p($file);
		if ($this->fileStore instanceof LtStore)
		{//p($this->fileStore->getFilePath($file));
			$cachedFileLastModified = (int) @filemtime($this->fileStore->getFilePath($file));
			if ($cachedFileLastModified < filemtime($file) || !is_array(($libNames = $this->fileStore->get($file))))//ps 第一次is_array(($libNames = $this->fileStore->get($file)) false
			{
				$libNames = $this->parseLibNames(trim(file_get_contents($file)));//ps  var_dump($libNames);  array(1) { ["class"]=> array(1) { [0]=> string(8) "LtConfig" } }
				//ps var_dump($libNames);   array(1) { ["class"]=> array(1) { [0]=> string(8) "LtConfig" } }
