...
class userInfoModel extends model
{
    ...
    */
    ...
    {
        $info = $this->model ('user_info')
                     ->select ('chk_idcard,id_type,id_card,real_name')
                     ->where ('user_id='.$user_id)
                     ->getRow ();
        if ($info['chk_idcard']) {
    ...
    */
     ...
     * @return array
    */
    public function getUserInfoExt ($user_id, $table = '')
    {//table=company/agent
        return $this->model ('user_info_ext'.$table)
                    ->where ('user_id='.intval ($user_id))
                    ->getRow ();
    }
    ...
    {
        return $this->model ('user_info_ext')
                    ->where ('user_id='.intval ($user_id))
                    ->update ($data) && $this->getNumRows () || $this->model ('user_info_ext')
                                                                     ->add ($data + compact ('user_id'));
    }
    ...
    public function setQuestions ($user_id, array $questions)
    {
        ...
        return (bool)$this->model ('user_question')
                          ->add ($data, true);
    }
    ...
    {
        return $this->model ('user_question')
                    ->where ('user_id='.$user_id)
                    ->getRow ();
    }
    ...
    {
        return $this->model ('stat_bill')
                    ->where ("user_id=$user_id AND month_id=$month_id")
                    ->getRow ();
    }
    ...
    */
    public function getRecommentCoins ($startTime = 0, $endTime = 0)
    {
        ...
        $list = $this->select ("ui.user_id,ui.ref_uid")
                     ...
                     ->order ('ui.ref_uid desc')//->group('ui.ref_uid')
                     ->getAll ();
    ...
    */
    public function getBynowUser ()
    {
        ...
        $list = $this->select ("ui.user_id")
                     ...
                     ->order ('ui.user_id desc')//->group('ui.ref_uid')
                     ->getAll ();
    ...
    {
        if (empty($paypasswd)) {
            throw new exception('请输入交易密码！', 40);
        }
        $uaccount = M ('user:userAccount')->getPk ($user_id);
        if (empty($uaccount['pay_passwd'])) {
            throw new exception('请先设置交易密码！', 41);
        }
    ...
    {
        return $this->model ('user_commission')
                    ->add ($data);
    }
    ...
    public function getQappUserInfo ($user_id)
    {
        return $this->model ('customer_contact con')
                    ...
                    ->leftjoin ("contact_info cin", 'cin.user_id = con.user_id')
