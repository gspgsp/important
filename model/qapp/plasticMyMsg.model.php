...
class plasticMyMsgModel extends model
{
    ...
    public function getMyComment($user_id,$page=1,$size=10,$sync=6){
        if(is_array($sync)){
            $where = " pur.sync in (".implode(',',$sync).") and pz.send_id=$user_id ";
        }else{
            $where = " pur.sync=$sync and pz.send_id=$user_id ";
        }

        $sql = "SELECT DISTINCT `pur`.`id`,`pz`.id as pzid, `pur`.`p_id`, `pur`.`user_id`, `pro`.`model`, `pur`.`unit_price`, `pur`.`store_house`, `fa`.`f_name`, `pur`.`type`, `pur`.`content`, `pur`.`input_time`
        ...
        foreach ($data['data'] as $key=> &$value) {
            ...
            $this->model('weixin_plasticzone')->where("id=".$value['pzid'])->update(array('is_read'=>1));
     ...
     * 获取直系留言
     */
    public function _getLiuYanDirectLine($id,$user_id,$page=1,$size=10){
        $where = "pur_id = $id and send_id = $user_id";
        $says = $this -> model('weixin_plasticzone')
                ->select('id,send_id as rev_id,user_id,is_read,content,input_time')
                ->page($page,$size)
                ->where($where)
                ->order('input_time desc')
                ->getPage();
        //p($says);showTrace();exit;
        return $says;
    }
