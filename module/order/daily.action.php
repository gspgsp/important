...
class dailyAction extends adminBaseAction {
	...
	public function _grid(){
		...
		$sortField = sget("sortField",'s','s_transport_time'); //排序字段
		$sortOrder = sget("sortOrder",'s','desc'); //排序
		//筛选
		$where .=' where 1 and s_transport_time>0 and p_transport_time>0';
		//战队筛选
		$team=sget('team','s');
		...
		if($sTime == 'sale_transport_time'){
			$where.=getTimeFilter('s_transport_time'); //时间筛选
			$sortField = sget("sortField",'s','s_transport_time'); //排序字段
		}elseif($sTime == 'pur_transport_time'){
			$where.=getTimeFilter('p_transport_time'); //时间筛选
			$sortField = sget("sortField",'s','p_transport_time'); //排序字段
		}
		//关键词搜索
	...
	$list = $this->db->getAll('SELECT * FROM ( SELECT * FROM ( SELECT 	(SELECT s_cus.`c_name` FROM `p2p_customer` s_cus WHERE (SELECT o2.`c_id` FROM `p2p_order` o2 WHERE o2.o_id=o.o_id)=s_cus.c_id) AS s_name,
		...
		WHERE sale.p_id >0 AND o.`join_id` > 0 AND o.`order_status`=2 AND o.`transport_status`=2) aa
		UNION 
	...
	$list_count = $this->db->getRow('SELECT count(*) as total,sum(s_xj) as s_xj,sum(s_num) as s_num,sum(profit) as profit,sum(ship) as ship,sum(p_num) as p_num,sum(p_xj) as p_xj from ( SELECT * from ( SELECT 	(SELECT s_cus.`c_name` FROM `p2p_customer` s_cus WHERE (SELECT o2.`c_id` FROM `p2p_order` o2 WHERE o2.o_id=o.o_id)=s_cus.c_id) AS s_name,
		...
		foreach($list as &$value){
			...
			$value['p_input_time']=$value['p_transport_time']>1000 ? date("Y-m-d H:i:s",$value['p_transport_time']) : '-';
