...
class qapiAction extends null2Action
{
    ...
    public function reginfo()
    {
        ...
        if (!$this->_chkmobile($mobile)) $this->error($this->err);
        $cache = cache::startMemcache();
        if (!$cache->get($mobile . 'check_reg_ok')) $this->error('已过期，请重新注册');
        if ($_GET) {
            if (!$name=sget('name', 's')) $this->error('请输入姓名');
            ...
            try {
                ...
                if ($old_user['user_id']) {
                    ...
                    if (!empty($_user['parent_mobile'])) {
                        ...
                        if(!M("qapp:pointsBill")->addPoints($this->rePoints, $focused_id, 12)) throw new Exception("系统错误 reg:112");
                    }
    ...
    public function getPlasticPerson()
    {
        ...
        if ($_GET) {
            ...
            if($user_id > 0){
                ...
                if($page==1&&!$this->db->from('log_login')->select('input_time')->where("input_time >".$dayTime." and chanel =6 and quan_type = 1 and user_id=$user_id")->getOne()){
...
//                 $where="p.model='{$value['model']}' and p.product_type={$value['product_type']} and f.f_name='{$value['f_name']}'";
                ...
                if ($pid) {
                    ...
                    try {
                        ...
                        if ($type == 2) {//报价
                            $spoints=intval(M('system:setting')->get('points')['points']['sale']);
                            if(!M("qapp:pointsBill")->select('id')->where("addtime >".strtotime(date("Y-m-d"))." and type=3")->order("id desc")->getOne()){
                                if (!$arr = M("qapp:pointsBill")->addPoints($spoints, $user_id, 3)) throw new Exception("系统错误 pubpur:103");
                            }
                        }elseif($type == 1){//采购
                            $spoints=intval(M('system:setting')->get('points')['points']['pur']);
                            if(!M("qapp:pointsBill")->select('id')->where("addtime >".strtotime(date("Y-m-d"))." and type=6")->order("id desc")->getOne()){
                                if (!$arr = M("qapp:pointsBill")->addPoints($spoints, $user_id, 6)) throw new Exception("系统错误 pubpur:103");
                            }
                        }
                    } catch (Exception $e) {
                        $pur_model->rollback();
    ...
    public function saveMsg()
    {
        ...
        if ($_GET) {
            ...
            if (empty($content)) $this->json_output(array('err' => 1, 'msg' => '回复内容不能为空'));
    ...
    {
        if ($count > 0) {
            if ($count % $size == 0 && ceil($count / $size) < $page) {p(1);exit;
                $this->json_output(array('err' => 3, 'msg' => '没有更多数据'));
            } elseif ($count % $size != 0 && ceil($count / $size) < $page) {p(2);exit;
                $this->json_output(array('err' => 3, 'msg' => '没有更多数据'));
            }
        }
    }
    ...
    public function exchangeSupplyOrDemand()
    {
        ...
        if ($_GET) {
            ...
            try{
                ...
                if($pointsRow['cate_id']==7) throw new Exception("系统错误 pubpur:116");
    ...
    protected function checkSupply($num=null,$outType=0){
        ...
        if($outType==0){
            ...
            if($pointsRow['cate_id']==7)  $this->json_output(array('err'=>116,'msg'=>'App内暂时不进行现金兑换，请于微信内搜索《塑料圈》进行兑换'));
    ...
    public function pointSupplyList(){
        ...
        if($_GET){
            ...
            $page = sget('page', 'i', 1);
            $size = sget('size', 'i', 10);
            $data=M("qapp:pointsBill")->select('id,addtime,type,points')->where("uid = $user_id and type in (3,5,6)")->order('id desc')->page($page,$size)->getPage();
            if (empty($data['data']) && $page == 1) $this->json_output(array('err' => 2, 'msg' => '没有相关数据'));
            $this->_checkLastPage($data['count'], $size, $page);
            //我的积分
            $points = M('qapp:pointsBill')->getUerPoints($user_id);
    ...
    public function exchangeList(){
        ...
        if($_GET) {
            ...
            $this->_checkLastPage($data['count'], $size, $page);
            foreach($data['data'] as $k => &$v){
                $v['name'] = M("points:pointsGoods")->where("goods_id =". $v['goods_id'])->select('name')->getOne();
    ...
    public function supplyDemandList(){
        ...
        if ($_GET) {
            ...
            foreach($data['data'] as $k => $v){
                ...
                if(!empty($v['model'])) $arr[$k]['content'] .= '牌号:'.$v['model'];
                if(!empty($v['store_house'])) $arr[$k]['content'].='厂家:'.$v['store_house'];
                if(!empty($v['unit_price'])) $arr[$k]['content'].='价格:'.$v['unit_price'];
                if(!empty($v['f_name'])) $arr[$k]['content'].= '交货地:'.$v['f_name'];
                if(!empty($v['content'])) $arr[$k]['content'].= $v['content'];
                $arr[$k]['p_id']=$v['id'];
