...
class indexAction extends adminBaseAction{
	...
	public function init()
	{
		...
		// 所有发布的信息，包括求购和销售
		$offerList = $this->model->where("status='上架'")->order("input_time desc")->getAll();
	...
	public function myload()
	{
		...
		$list = $this->db->model('share1')->where("`uname`='{$this->uname}'")->select('*')->order('input_time desc')->page($p,$size)->getAll();
