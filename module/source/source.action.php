...
class sourceAction extends adminBaseAction {
	public function __init(){
		...
		//$this->isSecurity = sget('isSecurity','i','');
	...
	public function info(){
		...
		if($user_id<1){
			if($cType==1){
				$this->assign('page_title','新增筹资机构个人');
				$this->display('user.add.html');
			}elseif($cType==2){
				$this->eco_type=L('eco_type'); //风险评估
				$this->assign('page_title','新增经纪人');
				$this->display('agent.add.html');
			}elseif($cType==3){
				$this->assign('page_title','新增筹资机构企业');
	...
	public function addSubmit() {
		...
		if($utype==1){
			...
			$real_name=$data['legal_person'];	 // '法人',
			$id_card=$data['id_card'];	 //'法人身份证',
			$id_type=(int)$data['id_type'];	
		}
		$info=array(
			'utype'=>$utype, //筹资机构
			...
			'id_type'=>$id_type,		
			'own_uid'=>$own_uid,
			'is_security'=>empty($data['is_security'])?'0':$data['is_security'],
		);
		
		$info_ext = $user_att = array();
		if($data['utype']==2){//2经纪人
			$info_ext = array(
				'name'=>$data['name'],	 //'公司名称',
				...
				'zip'=>$data['zip'],	 //'邮编',
				'customer_code'=>$data['customer_code'],	 //客服经理代码
				'ecotype'=>$data['ecotype'],	 //'机构类型
			);
		}elseif($data['utype']==3){//3公司
			$user_att = $data['user_att'];//TODO:
			$info_ext = array();
			foreach($data as $k=>$v){
		...
		$result=M('system:sysUser')->register($param,$info,$info_ext);
		if($result['err']>0){
