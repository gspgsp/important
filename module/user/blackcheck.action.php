 ...
 * 客户信息管理
 */
class blackcheckAction extends adminBaseAction {
	...
	public function _grid(){
		...
		$where = '( `is_sale` = 2 or `is_pur` = 2 or `status` = 10 )';
	...
	public function check(){
		...
		$action=sget('action','s');
		if ($action==1 && is_array($data)) {
			foreach ($data as $k => $v) {
				$res = M('user:customer')->getColByName($v,"c_id","c_id");
				if($res>0){
				//加入黑名单
					$result=$this->db->model('customer')->where("`c_id`=$v")->update(array('status'=>9,'is_sale'=>0,'is_pur'=>0,));
				//新增客户流转记录日志----S
					$remarks = "对客户操作：将待审黑名单客户加入黑名单";// 审核用户
					M('user:customerLog')->addLog($v,'check','黑名单待审核','黑名单审核通过',1,$remarks);
				//新增客户流转记录日志----E
				}
			//查询是不是有该用户的报价
			...
			foreach ($data as $k => $v) {
				...
				if($res>0){
				//加入黑名单
					$che_data = $this->db->model('customer')->select('is_sale,is_pur')->where("`c_id`=$v")->getRow();
					if ($che_data['is_sale']==2 && $che_data['is_pur']==2)  {
						$result=$this->db->model('customer')->where("`c_id`=$v")->update(array('is_sale'=>1,'is_pur'=>1,));
					}elseif ($che_data['is_sale']==2 && $che_data['is_pur']!=2) {
						$result=$this->db->model('customer')->where("`c_id`=$v")->update(array('is_sale'=>1,'is_pur'=>$che_data['is_pur'],));
					}elseif ($che_data['is_sale']!=2 && $che_data['is_pur']==2) {
						$result=$this->db->model('customer')->where("`c_id`=$v")->update(array('is_sale'=>$che_data['is_sale'],'is_pur'=>1,));
					}else{
						$result=$this->db->model('customer')->where("`c_id`=$v")->update(array('status'=>2,'is_sale'=>$che_data['is_sale'],$che_data['is_pur'],));
					}
				//新增客户流转记录日志----S
					$remarks = "对客户操作：加入黑名单审核未通过，客户回滚";// 审核用户
					$s= M('user:customerLog')->addLog($v,'check','黑名单待审核客户','客户原始状态',1,$remarks);
				//新增客户流转记录日志----E
				}
			}
		}
