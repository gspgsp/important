...
class purchaseAction extends adminBaseAction {
	...
	public function init(){
		...
		$this->assign('period',L('period'));//周期
	...
	private function _grid(){
		...
		//关键词搜索
		$key_type=sget('key_type','s','period');
		$keyword=sget('keyword','s');
		if(!empty($keyword)){
			$where.=" and `$key_type`='$keyword' ";
		}
		$list=$this->db->select("p.*,pd.p_name, pd.cate_id, pd.model, pd.f_name, pd.f_id, pd.product_type, pd.process_type, pd.unit")
		...
		foreach($list['data'] as $k=>$v){
			...
			$list['data'][$k]['cate_id'] = L('product_type')[$v['cate_id']];
			$list['data'][$k]['f_id'] = $this->getFactoryName($v['f_id']);
	 ...
	 * 根据厂家id获取厂家名称
	 */
	private function getFactoryName($id){
