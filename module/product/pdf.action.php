...
class pdfAction extends adminBaseAction {
	...
	public function pdf(){
		...
		$ordertitle = $orderLists['order_name'] == 1 ? '上海中晨电子商务股份有限公司' : '上海梓辰实业有限公司';
		$shipname = $orderLists['transport_type'] == 1 ? '提货' : '送货';
		$orderList=!empty($orderLists)?$orderLists:'';
		$detiles=!empty($detiless)?$detiless:'';
		//提货日期
		...
		public function outpdf(){
			$arr = array(
				'pdf_template'=>array(
					'out'=>'<table width="635" border="0" align="center" bgcolor="#fff">
							<tr ><td>&nbsp;</td></tr>
							<tr >
								<td colspan="2" align="center"><h1>上海中晨电子商务股份有限公司</h1></td>
							</tr>
			...
			$table5='
				...
				<tr>
					...
					<td align="right" height="20" style="line-height:20px;"  width="140">&nbsp;客户名（签章）：</td>
			...
			';
			// 出库信息表
			$str = sprintf($arr['pdf_template']['out'],$oinfo['order_sn'],date('Y-m-d',$info['input_time']),$table4,$table5);
