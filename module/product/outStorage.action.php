...
class outStorageAction extends adminBaseAction {
	public function __init(){
		...
		$this->assign('type',array('1'=>'出库','2'=>'入库'));//出入库类型
	...
	public function info(){
		...
		if($action=='grid'){
			$where = "`o_id`=".$o_id;
			$list=$this->db->model('storage_log')->where($where)
					->page($page+1,$size)
	...
	public function remove(){
		...
		if(is_array($data)){
			foreach ($data as $k => $v) {
				$result=$this->db->model('storage_log')->where("id in ($ids)")->delete();
			}
		}
		if($result){
			$this->success('操作成功');
	...
	public function save(){
		...
		foreach($data as $k=>$v){
			...
			$diff_num=M("product:outStorage")->checkNum($v['detail_id'],$v['number']); //订单和出库数量比较
			if(!$diff_num) $this->error('数量有误');
			$result=$this->db->model('storage_log')->wherePk($v['id'])->update($_data);
		}
		if($result){
			$this->success('操作成功');
		}else{
