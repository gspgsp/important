...
<div class="mini-fit" style="padding:1px 3px 3px;">
	<div id="gridCell" class="mini-datagrid" style="width:100%;height:100%;"url="/product/storeDetail/logDetail?action=grid&do=<?php echo $this->_var['doact']; ?>&id=<?php echo $this->_var['id']; ?>"  idField="id"
	sizeList="[10,20,50,100]" pageSize="20" multiSelect="true" showFilterRow="true" allowCellSelect="true" allowCellEdit="true" >
		<div property="columns">
			...
			<div field="purchase_id" width="60" headerAlign="center" align="center" renderer="onLoadHandle">采购明细</div>
			<div field="number" width="55" headerAlign="center" align="center">单次入库数</div>
			<div field="remainder" width="55" headerAlign="center" align="center" >剩余数量</div>
			<div field="inlog_id" width="55" headerAlign="center" align="center" >库存明细</div>
...
function onLoadHandle(e) {
	var record = e.record,s='',purchase_id = record.purchase_id;
	s += '<a href="javascript:viewOrdInfo('+purchase_id+')">'+purchase_id+'</a> ';
	return s;
}
//查看订单相关信息
...
function release() {
	...
	var ids=_ids.join(',');
	if(ids=='') return;
	mini.confirm("确定撤销该入库流水吗？</br>1：设置后该记录会被标记无效</br>2：产生的库存记录会被返还</br>3：订单相关的内容会被撤销", "提示",	function(action){
			if(action!='ok') return;
			...
			utils.ajax('/product/storeDetail/instoreBack',{ids:ids},callback,"POST","json");
		}
	);
}
