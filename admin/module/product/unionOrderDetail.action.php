 ...
 * 联营订单详情
 */
class unionOrderDetailAction extends adminBaseAction {
	public function __init(){
		$this->debug = false;
		$this->db=M('public:common')->model('union_order_detail');
	...
	public function init(){
		...
		$this->display('union_order_detail.list.html');
	}
	/**
	 * Ajax获取列表内容
	 * @access private 
	...
	private function _grid(){
		...
		if(!empty($keyword) && $key_type=='o_id'){
			$keyword=M('product:unionOrder')->getidByOname($keyword);
		...
		foreach($list['data'] as $k=>$v){
			// 查询付订单的订单详情
			$tailInfo = M("product:unionOrder")->getAllInfoById($v['o_id']);
