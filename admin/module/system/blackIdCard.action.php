 ...
 * 身份证黑名单
 */
class blackIdCardAction extends adminBaseAction {
	public function __init(){
		$this->debug = false;
		$this->action = sget('action','');
		$this->db=M('public:common')->model('black_idcard');
	}
	...
	public function init(){
		...
		$this->assign('page_title','身份证黑名单');
		$this->display('black_idcard.html');
	}
	...
	public function ajaxSave(){
		...
		//检查是否存在
		$exist=$this->db->select('id')->where("idcard='$data[idcard]'")->getOne();
		if($exist){
			$this->error('此身份证已经在黑名单中，请重新输入');	
		}
