 ...
 * 国家基础库管理
 */
class libCountryAction extends adminBaseAction {
	public function __init(){
		$this->debug = false;
		$this->action = sget('action','');
		$this->db=M('public:common')->model('lib_country');
	}
	...
	public function init(){
		...
		$this->assign('pid',$pid);
		$this->assign('page_title','国家地区设置');
		$this->display('lib_country.html');
	}
	...
	public function save(){
		...
		//检查是否存在统计的模块名
		$names=$this->db->select('zh_cn')->where('pid='.$data[0]['pid'])->getCol();

		foreach($data as $k=>$v){
			$_id=$v['id'];
			if($_id<1 && !empty($names) && in_array($v['zh_cn'],$names)){
				$this->error('第['.($k+1).']行不能有相同模块名['.$v['zh_cn'].']');
			}
			$names[]=$v['zh_cn']; //追加检查模块名
			unset($v['id']);
			if($_id>0){
