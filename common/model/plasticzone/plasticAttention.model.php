...
class plasticAttentionModel extends model
{
	...
	public function getAttention($userid,$focused_id){
		...
		if(empty($result)){
			...
			if($this->model('weixin_fans')->add($_data)) return array('err'=>0,'msg'=>'关注成功');
			...
			if($bools=$this->model('weixin_fans')->where("user_id=$userid and focused_id=$focused_id")->update($_data)){
				if($result['status']==2) return array('err'=>0,'msg'=>'关注成功');
				if($result['status']==1) return array('err'=>0,'msg'=>'取消关注成功');
			}
			return array('err'=>2,'msg'=>'关注/取消关注失败');
		}
	}
	//发送提醒短信
