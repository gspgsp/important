		...
		<li v-show="condition" v-for="n in name">
			<div class="supplytitle">
				<h3>{{n.input_time}}<span v-on:click="del(n.id)">删除</span>
					<a v-link="{name:'supplybuy',params:{id:n.id}}" style="float: right; margin: 0 10px 0 0; color: #62759e;">分享</a>
				</h3>
			...
			<div class="supplycontent">
				...
				<p>Ta在卖：{{n.b_and_s[1].c_name}}<br><span style=" display: inline-block; width: 48px; height: 1px;"></span><a v-link="{name:'personinfo',params:{id:n.b_and_s[0].user_id}}">{{n.b_and_s[1].name}}</a> {{n.b_and_s[1].mobile}}</p>
        ...
        methods:{
        	del:function(id){
        		...
        		mui.confirm('', '确定删除此条信息？', ['取消', '确定'], function(e) {
                    if (e.index == 1) {
	                    _this.$http.post('/plasticzone/plastic/deleteMyMsg',{
		        			id:id
		        		}).then(function(res){
        ...
        ready:function () {
        	...
        	$(window).scroll(function() {
	            ...
	            if (scrollTop + windowHeight == scrollHeight) {
	            	_this.page++;
		           	_this.$http.post('/plasticzone/plastic/getMyMsg',{
            ...
            this.$http.post('/plasticzone/plastic/getMyMsg',{
