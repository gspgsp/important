		...
		<li v-show="condition" v-for="n in name">
			...
			<div class="supplycontent">
				...
				<div class="replyRelease2" v-if="n.says.length!==0">
					...
					<p v-for="n2 in n.says" v-if="n.user_id!==n2.rev_id">
						<router-link :to="{name:'personinfo',params:{id:n2.user_id}}">{{n2.user_name}}</router-link>回复<router-link :to="{name:'personinfo',params:{id:n2.rev_id}}">{{n2.rev_name}}</router-link>:<i v-on:click="reply(n.id,n2.user_id,n2.id)">{{n2.content}}</i>
					</p>
				</div>
			</div>
		</li>
